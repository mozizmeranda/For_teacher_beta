...
async def theme(message: types.Message, state: FSMContext):
    ...
    if message.photo:
        async with state.proxy() as data:
            data['theme'] = message.photo[0].file_id
        await message.answer(F_language(answer="Теперь пришлите сам вопрос.",
                                        language=db.get_language(id=message.from_user.id)))
        await Questionioning.next()
...
async def get_question(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        if len(data['theme'])<15:
            ...
            data['question'] = message.text
            await bot.send_photo(chat_id=message.from_user.id, photo=data['theme'], caption=data['question'], reply_markup=button(language=db.get_language(id=message.from_user.id)))
        await Questionioning.next()
    ...
    async with state.proxy() as data:
        if len(data['theme']) < 15:
            ...
            for i in ADMINS:
                ...
                await bot.send_photo(chat_id=i, photo=data['theme'], caption=question, reply_markup=question_button(question_code=code))
    await call.answer(text=F_language(answer="Вопрос успешно отправлен.", language=db.get_language(id=call.from_user.id)))
    await state.finish()
